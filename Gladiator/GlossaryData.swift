        ...
        GlossaryTerm(
            ...
            children: [
                ...
                GlossaryChild(
                    ...
                    definition: "How Race Engineer frames Setup Recommendations when one metric is clearly the largest single lever in the winning model. Two conditions must hold: the largest absolute standardized regression coefficient (|β|) must be at least twice the size of the next largest, and that top |β| must itself clear an absolute floor of 0.15 — a small-but-real standardized effect. The floor exists because a 2× ratio between two near-zero coefficients can fire on sampling noise alone, so without it a noisy model could be labelled dominant on essentially nothing. When both conditions are met, the recommendation copy tells you to focus your adjustment on that one metric for the most direct impact on the outcome, the dominant metric is tagged with a small Primary badge in the recommendation list, and the existing 'one change at a time, start with the highest contributor' guidance is retained — because the data supports treating that single metric as the priority lever. If either condition fails, Race Engineer falls back to the Weighted Mix framing instead."
                ),
                GlossaryChild(
                    id: "weighted-mix",
                    name: "Weighted Mix",
                    definition: "How Race Engineer frames Setup Recommendations when no single metric stands out as a dominant lever — the largest absolute standardized regression coefficient (|β|) is less than twice the next largest, so several metrics carry similar weight. In this mode the recommendation copy advises adjusting the metrics proportionally rather than focusing on one, since changing only one may unbalance the relationship the model picked up. The reference is the predictor with the largest |β| in the winning model. For every other predictor Race Engineer reports a proportional adjustment ratio in raw units, computed from the unstandardized regression slopes (b = β × SD of outcome ÷ SD of predictor). The displayed ratio is the number of the predictor's own units whose outcome contribution equals one unit of the reference, so it represents an equivalent change in the outcome rather than a like-for-like β share. The direction is sign-aware: when the two predictors' coefficients have the same sign, adjusting them the same way produces a matched outcome change; when their signs differ, the equivalent change requires moving them in opposite raw-unit directions."
                ),
                GlossaryChild(
                    id: "elasticity-warning",
                    name: "Elasticity Warning",
                    definition: "A caution shown beneath a Setup Recommendation when the recommended target sits within 15% of the edge of the data you have actually logged. The anchor depends on which form the recommendation took: when Race Engineer is showing a target range from your fastest sessions, the leading edge of that range in the recommended direction is the anchor (the upper edge for upward recommendations, the lower edge for downward); when it is instead advising you to raise or lower from your current session average, the average itself is the anchor. The note flags that the linear relationship the model fit was estimated inside your observed range and may not continue to hold beyond it, so changes carrying you past that edge should be made cautiously and verified on track before larger adjustments. The warning is suppressed when direction is uncertain (because no actionable direction is being recommended in that case) and when the target sits comfortably inside the observed range."
                ),
                GlossaryChild(
                    id: "data-sufficiency",
                    name: "Data Sufficiency",
